...
def _run_EFF(script):
    """Le corps de `run_EFF`, et l'arbre de la fonction."""
    s = io.open(os.path.join(_REPO, script), encoding="utf-8",
                errors="replace").read()
    main = [n for n in ast.parse(s).body if isinstance(n, ast.If)][0]
    return {f.name: f for f in main.body
            if isinstance(f, ast.FunctionDef)}["run_EFF"]
...
def test_le_ratio_du_bilan_est_defini_avant_la_boucle(script):
    """L'invariant, verifie sur l'arbre : `_ratio_bb` doit etre affecte
    quelque part AVANT le `while`, pas seulement dedans."""
    f = _run_EFF(script)
    boucles = [n for n in ast.walk(f) if isinstance(n, ast.While)]
    assert len(boucles) == 1, (script, len(boucles))
    ...
    affectations = [n.lineno for n in ast.walk(f)
                    ...
                    for c in n.targets
                    for x in ast.walk(c)
                    if isinstance(x, ast.Name) and x.id == "_ratio_bb"]
    assert affectations, "%s : `_ratio_bb` n'est plus affecte du tout" % script
    assert min(affectations) < debut_boucle, (
        "%s : `_ratio_bb` n'est affecte qu'a partir de la ligne %d, alors que "
        "la boucle commence l. %d et que le bilan le lit APRES. Un run qui "
        "n'entre jamais dans la boucle -- budget epuise a la reprise, ou "
        "critere EFF deja satisfait -- leve UnboundLocalError."
        % (script, min(affectations), debut_boucle))


@pytest.mark.parametrize("script", ETUDES)
def test_toute_variable_lue_apres_la_boucle_est_definie_avant(script):
    ...
    assert not fragiles, (
        "%s : %s n'est affecte que dans la boucle d'enrichissement et lu "
        "apres elle. Si la boucle ne tourne pas -- budget deja epuise a la "
        "reprise, critere EFF deja satisfait -- c'est un UnboundLocalError "
        "au moment du bilan." % (script, ", ".join("`%s`" % n for n in fragiles)))
...
# --------------------------------------------------------------------------- #
@pytest.mark.parametrize("script", ETUDES)
def test_le_bilan_BB_n_est_ecrit_qu_une_fois(script):
    ...
    assert s.count("[BB informatif final] ratio =") == 1, (
        "%s : la ligne de bilan BB est ecrite plusieurs fois." % script)
