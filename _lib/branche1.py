"""
Coquille de compatibilite : `branche1` a ete renomme en `api.py`.

PHASE 2 du plan de nettoyage (docs/plan-nettoyage.md). Aucun code ici : le
module delegue a `api`.
...
Pourquoi `__getattr__` plutot que `from api import *` : la deleguation est
...
import api as _cible

_warnings.warn(
    "Le module 'branche1' est renomme 'api'. L'ancien nom sera retire a la "
...
def __getattr__(nom):
    """Deleguation tardive : resout dans `api` au moment de l'acces."""
    try:
        return getattr(_cible, nom)
    except AttributeError:
        raise AttributeError(
            "module 'branche1' (renomme 'api') n'a pas d'attribut %r" % nom) from None
