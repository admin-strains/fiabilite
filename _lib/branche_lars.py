"""
Coquille de compatibilite : `branche_lars` a ete renomme en `lars.py`.

PHASE 2 du plan de nettoyage (docs/plan-nettoyage.md). Aucun code ici : le
module delegue a `lars`.
...
Pourquoi `__getattr__` plutot que `from lars import *` : la deleguation est
...
import lars as _cible

_warnings.warn(
    "Le module 'branche_lars' est renomme 'lars'. L'ancien nom sera retire a la "
...
def __getattr__(nom):
    """Deleguation tardive : resout dans `lars` au moment de l'acces."""
    try:
        return getattr(_cible, nom)
    except AttributeError:
        raise AttributeError(
            "module 'branche_lars' (renomme 'lars') n'a pas d'attribut %r" % nom) from None
