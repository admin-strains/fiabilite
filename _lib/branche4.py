"""
Coquille de compatibilite : `branche4` a ete renomme en `predict.py`.

PHASE 2 du plan de nettoyage (docs/plan-nettoyage.md). Aucun code ici : le
module delegue a `predict`.
...
Pourquoi `__getattr__` plutot que `from predict import *` : la deleguation est
...
import predict as _cible

_warnings.warn(
    "Le module 'branche4' est renomme 'predict'. L'ancien nom sera retire a la "
...
def __getattr__(nom):
    """Deleguation tardive : resout dans `predict` au moment de l'acces."""
    try:
        return getattr(_cible, nom)
    except AttributeError:
        raise AttributeError(
            "module 'branche4' (renomme 'predict') n'a pas d'attribut %r" % nom) from None
