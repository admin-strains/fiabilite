...
if __name__ == '__main__':
    ...
    def build_DOE(n_doe=n0, eval_hf=True):
        ...
        T     = dist_X.getIsoProbabilisticTransformation()
        T_inv = dist_X.getInverseIsoProbabilisticTransformation()
        # Le domaine de tirage suit les bornes de l'etude. Il etait CODE EN
        # DUR a +/- 7,5 ici et lu dans `eff_bounds` dans l'autre etude :
        # borner le domaine n'avait donc d'effet que d'un cote. Les deux
        # coincident aujourd'hui (eff_bound = +/- 7,5 sur cette etude),
        # donc rien ne bouge -- mais la prochaine fois qu'on bornera, si.
        dist_U = ot.JointDistribution(
            [ot.Uniform(eff_bounds_min[i], eff_bounds_max[i])
