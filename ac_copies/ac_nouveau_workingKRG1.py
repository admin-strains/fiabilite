...
if __name__ == '__main__':
    ...
    n0 = max(15,n_multistart) # pour avoir n0>=n_multistart
    ...
    # A MODIFIER POUR FORMAT EVENT 
    def FORM_event_KRG(metamodel):
    ...
    if do_GP and not try_pce and do_GEK: # remettre elif apres decommentage. 
    # --------------------------------------------------------------------------- #
    # MODELE GEK PUR                                                              #
    # --------------------------------------------------------------------------- #
        xt, yt, all_grad = build_DOE()
        sm = build_metamodel_GEK(xt, yt, all_grad)
        g_ot = ot.Function(GEKPLSFunction(sm))
        event = FORM_event_GEK(g_ot)
        ...
        if best_result is None:
            print('Aucun FORM ne marche.')
        else: 
            print(f"beta = {best_result.getHasoferReliabilityIndex():.4f}")
            print(f"Pf   = {best_result.getEventProbability():.4e}")
            print(f"u*   = {[round(v, 4) for v in best_result.getStandardSpaceDesignPoint()]}")
            print(f"Imp. = {[round(v, 4) for v in best_result.getImportanceFactors()]}")
            import matplotlib.pyplot as plt
            from matplotlib.lines import Line2D

            # --- Grille surrogate fine (100x100, pas d'appels HF) ---
            n_grid = 100
            u1 = np.linspace(-4, 4, n_grid)
            u2 = np.linspace(-4, 4, n_grid)
            U1, U2 = np.meshgrid(u1, u2)
            grid = np.column_stack([U1.ravel(), U2.ravel()])
            Z_surr = sm.predict_values(grid).reshape(n_grid, n_grid)

            # --- Grille HF grossière (6x6 = 36 appels run_HF) ---
            n_grid_hf = 6
            u1_hf = np.linspace(-4, 4, n_grid_hf)
            u2_hf = np.linspace(-4, 4, n_grid_hf)
            ...
            plt.colorbar(cf, ax=ax, label='g (surrogate)')
            ...
            ax.scatter(u_star[0], u_star[1],   c='gold',   s=200, zorder=8, marker='*',
                    label=f'u* beta={best_result.getHasoferReliabilityIndex():.3f}')

            legend_extra = [
                Line2D([0], [0], color='red',  linestyle='--', linewidth=2, label='g=0 HF'),
                Line2D([0], [0], color='blue', linestyle='-',  linewidth=2, label='g=0 surrogate'),
            ]
            ax.legend(handles=ax.legend().legend_handles + legend_extra)
