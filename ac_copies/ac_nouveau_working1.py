"""
Ce fichier a pour différence le form multistart.  
"""
import os
import json
import shutil
...
def dist_jointe(modelname, params_names):
    dist = []
...
def run_HF(modelname, params_names, u, sensitivity=True):
    n_var = len(u)
...
if __name__ == '__main__':
    ...
    # OPTIONS FORM                                                                #
    n_max_FORM = 50
    tol_FORM = 0.2                                    # précision acceptée par FORM
    do_warm_start = False
    tol_warm_start = 0.0001                      # nécessité de faire le warm_start
    tol_multistart = 0.01                             #comparaison entre deux modes
    do_multistart = True
    n_multistart = 1
    ...
    # 1. GP
    params_names = ['fc','fy']
    n0 = max(25,n_multistart) # pour avoir n0>=n_multistart
    ...
    # 3. PCE                                                                
    do_pce = try_pce                                     #booléen de validation pce
    ...
    # if U_doe_fixed is None:
    #     ot.RandomGenerator.SetSeed(int(time.time()))
    print_DOE = True
    # --------------------------------------------------------------------------- #
    ...
    def fill_sol(modelname, params_names, U_doe):
        n_var = U_doe.getDimension()
        n0 = U_doe.getSize()
        SOL_U = [{} for _ in range(n0)]
        for i in range(n0):
            for j in range(n_var):
                SOL_U[i][f'u{j+1}'] = U_doe[i][j]
        for i, u in enumerate(U_doe):
            g_HF, grad_HF_U, grad_HF_X = run_HF(modelname, params_names, u)
            SOL_U[i]['g'] = g_HF
            for j in range (n_var):
                SOL_U[i][f'dg_u{j+1}'] = grad_HF_U[j]
                SOL_U[i][f'dg_{params_names[j]}'] = grad_HF_X[j]
        return SOL_U
    
    def init_GP(modelname, params_names, n0, U_doe_fixed = None):
        n_var = len(params_names)
        if U_doe_fixed is not None:
            U_doe = U_doe_fixed
        else:
            U_doe = tirage_DOE(modelname, params_names, n0)
            if print_DOE:
                ...
                for i in range(U_doe.getSize()):
                    ...
                    print(f"    [{', '.join(vals)}],")
                print("])")
        SOL_U = fill_sol(modelname, params_names, U_doe)
        xt = np.array(U_doe)
    ...
    def build_hf_cache(modelname, params_names):
        class HFCache:
            def __init__(self):
                self._last_u = None
                self._last_g = None
            ...
            def run_if_needed(self, u):
                ...
                if self._last_u is None or u_list != self._last_u:
                    self._last_g, self._last_grad, _ = run_HF(modelname, params_names, u)
                    self._last_u = u_list
        hf_cache = HFCache()
        return hf_cache

    # --------------------------------------------------------------------------- #
    # FONCTIONS LIEES AU PCE                                                      #

    def build_metamodel_PCE(modelname, params_names, xt, y_hf, q = q, max_degree = max_degree, seuil_pce= seuil_pce, min_max_degree = min_max_degree):
    ...
    def fill_PCE(modelname, params_names, xt, metamodel_PCE):
        ...
        return y_PCE, all_grad_PCE, all_sensib_PCE
    
    def fill_inputGP(y_hf, all_grad_hf, all_sensib_hf, y_PCE, all_grad_PCE, all_sensib_PCE, do_pce):
        yt = y_hf.copy()
    ...
    def build_metamodel_GEK(xt, yt, all_grad): # ATTENTION RENOMME SANS VERIFICATION OPTION PAR DEFAUT ENLEVEE
        n_var = xt.shape[1]
    ...
    def FORM_KRG(modelname, params_names, metamodel, start_point):
        n_var = len(params_names)
        dist_X = dist_jointe(modelname, params_names)
        dist_U = dist_X.getStandardDistribution()
        vect = ot.RandomVector(dist_U)
        output = ot.CompositeRandomVector(metamodel, vect)
    ...
    def FORM_GEK(modelname, params_names, metamodel, start_point):
        dist_X = dist_jointe(modelname, params_names)
        dist_U = dist_X.getStandardDistribution()
        n_var = len(params_names)
        ...
        solver.setCheckStatus(False)
        solver.setMaximumConstraintError(tol_FORM)
        solver.setStartingPoint(start_point)  # point de depart = moyenne en U
        algo = ot.FORM(solver, event)
        algo.run()
        result = algo.getResult()
        return result, event
    ...
    def FORM_multistart(starting_points, event): #cette fonction cherche le meilleur mode et non tous les modes
        best_beta = np.inf                # une fois qu'elle fonctionne, en faire une qui sort tous les modes qui sont ok par rapport à une tolérance au lieu du meileur. 
        best_result = None
        ...
        for sp in starting_points:
            try:
                ...
                beta_i = r_i.getHasoferReliabilityIndex()
                print(f"start={sp}  ->  beta={beta_i:.4f}  u*={[round(v,3) for v in r_i.getStandardSpaceDesignPoint()]}")
                if beta_i < best_beta:
                    best_beta = beta_i
                    best_result = r_i
            except Exception as e:
                print(f"start={sp}  ->  ECHEC ({e})")
        if best_result is None:
            print(f'Aucun FORM ne converge')
        else: 
            print(f"\nbeta_min = {best_result.getHasoferReliabilityIndex():.4f}")
            print(f"Pf_min   = {best_result.getEventProbability():.4e}")
            print(f"u*       = {[round(v,4) for v in best_result.getStandardSpaceDesignPoint()]}")
        return best_result
    ...
    # 2. FORM                                                                     # 
        start_point = [0.0]*len(params_names)
        result, event = FORM_GEK(modelname, params_names, metamodel, start_point)
        U_warm = result.getPhysicalSpaceDesignPoint()
        if do_warm_start and metamodel(U_warm)[0] > tol_warm_start:
            U_doe = ot.Sample(xt)
            U_doe.add(U_warm)
    ...
    elif do_GP and not try_pce and do_GEK:
    # --------------------------------------------------------------------------- #
    # MODELE GEK PUR                                                              #
    # --------------------------------------------------------------------------- #
    # 1. METAMODELE                                                               #
    ...
    # 2. FORM                                                                     #
        ...
        n_iter, U_res, X_res, g_GP_res, grad_res, importance, beta, Pf_FORM = resultats_GP(modelname, params_names, result, metamodel)
        if do_multistart:
            best_result = FORM_multistart(xt, event)
    # --------------------------------------------------------------------------- #
    # 3. AFFICHAGE                                                                #
        print_resultats(n_iter, U_res, X_res, g_GP_res, grad_res, importance, beta, Pf_FORM)
        print_GP_tests(modelname, params_names, U_res, g_GP_res, metamodel)
    ...
    #     # --------------------------------------------------------------------------- #
    #     # ETAPE 2 - FORM SUR LE METAMODELE                                            #
    #     # --------------------------------------------------------------------------- #
    #     if do_GP:
    #         pass
    #     else:
    ...
    #         if n_multistart >1:
    #             """
    #             On lance n_multistart<=n0 autres fois le FORM. On s'arrête dès
    ...
    #         import numpy as np
    #         sm = build_metamodel_GEK(xt, yt, all_grad, reduc_PLS = reduc_PLS)
    ...
    #     if do_GP and try_pce and do_pce_eval:
    #         def compute_q2_loo(inputSample, outputSample, dist_U, q, max_degree, seuil_pce):
    #             n = inputSample.getSize()
    #             y = outputSample[:, 0]
    #             y_mean = y.computeMean()[0]
    ...
    #         inputSample = U_doe
    #         outputSample = ot.Sample([[SOL_U[i]['g']] for i in range(n0)])
    #         q = 0.75
    #         max_degree = 2
    #         q2_loo = compute_q2_loo(inputSample, outputSample, dist_U, q, max_degree, seuil_pce)
    ...
    #             for j, u_fc in enumerate(u_fc_values):
    #                 u_scan = ot.Point([u_fc, u_fy])
    #                 g_val, _, _ = run_HF(modelname, params_names, u_scan)
    #                 G[i, j] = g_val
    ...
    #         print("Visu sauvegardée : etat_limite.png")
        ...
        Dans le code ci dessus, je défini do_pce, do_GP, et do_GEK pour qu'on puisse choisir le type
        ...
        # def result_PCE(inputSample, outputSample, dist_U, q = q, max_degree = max_degree, seuil_pce= seuil_pce, min_max_degree = min_max_degree):
